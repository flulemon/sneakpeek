...
coverage: $(PY_INSTALL_STAMP) ##Run tests
	$(POETRY) run pytest --cov=sneakpeek tests --cov-fail-under=70 --cov-report term-missing --cov-report html --cov-report xml
...
build-docs: $(PY_INSTALL_STAMP) ##Build documentation
	...
	$(POETRY) run sphinx-build $(ROOT_DIR)/docs $(ROOT_DIR)/sneakpeek/static/docs/
