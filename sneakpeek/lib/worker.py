...
from sneakpeek.lib.runner import RunnerABC
...
class Worker(WorkerABC):
    ...
    async def _execute_scraper(self, scraper_run: ScraperRun) -> None:
        self._logger.info(f"Starting executing scraper: {scraper_run}")
        try:
            await self._runner.run(scraper_run)
        except Exception as e:
            self._logger.error(f"Failed to execute {scraper_run}: {e}")
            self._logger.debug(f"Failed to execute {scraper_run}: {format_exc()}")
    ...
    async def _on_tick(self) -> None:
        async with self._lock:
            ...
            self._logger.info(f"Dequeued a job to run: {dequeued}")
